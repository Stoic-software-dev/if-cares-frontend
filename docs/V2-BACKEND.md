...
| `POST /api/meal-counts/approve` · `PUT` to undo | **Built 31-aug-2026.** Sets `approvedAt/approvedById/approvedByEmail`, blocks corrections (409, saying whether to undo the approval or void the count), leaves voiding available and does not gate reports. Mails the PDF to the staff assigned to that site and archives the same copy in Drive. Summer splits this into a second call because Apps Script needed 30-45s for it; here one site-day PDF plus a handful of emails runs inline, which a request that ends is the only thing this runtime promises to finish. A failure there is alerted and never undoes the approval. |
