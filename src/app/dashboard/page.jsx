...
import { MOCK_MONTH, MOCK_SITE } from '@/lib/mock-data';
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <main className="mx-auto flex max-w-md flex-col gap-4 px-4 pb-6 pt-5">
        <button
          type="button"
          className="flex h-[46px] items-center justify-between rounded-[10px] border border-slate-300 bg-white px-3.5"
        >
        ...
        <div className="flex items-center justify-between">
          <div className="flex flex-col">
            <h1 className="text-2xl font-bold tracking-tight text-slate-900 leading-tight">{MOCK_MONTH.label}</h1>
            <span className="text-[13px] font-medium text-slate-500">{MOCK_MONTH.year}</span>
          </div>
          <div className="flex gap-2">
            <button type="button" aria-label="Previous month" className="flex h-11 w-11 items-center justify-center rounded-[10px] border border-slate-200 bg-white text-slate-700">
              <ChevronLeft className="h-[18px] w-[18px]" />
            </button>
            <button type="button" aria-label="Next month" className="flex h-11 w-11 items-center justify-center rounded-[10px] border border-slate-200 bg-white text-slate-700">
        ...
        <Button
          onClick={() => router.push(`/meal-count?date=${MOCK_MONTH.todayDate}`)}
          className="mt-1 h-[52px] rounded-xl text-[15px] font-semibold"
        >
